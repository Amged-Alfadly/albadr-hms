...
// Dynamically determine the role based on the folder or session
$current_role = $_SESSION['role'];
$module_name = basename(dirname($_SERVER['PHP_SELF']));

// Security check: Ensure user is visiting the correct module for their role
// (Allow Admin to visit any, but others must match their role)
if ($current_role !== 'Admin' && strtolower($current_role) !== strtolower($module_name)) {
    // Some roles might have slightly different folder names
    $allowed = false;
    if ($current_role === 'Reception' && $module_name === 'reception') $allowed = true;
    if ($current_role === 'Doctor' && $module_name === 'doctor') $allowed = true;
    if ($current_role === 'Lab' && $module_name === 'lab') $allowed = true;
    if ($current_role === 'Admissions' && $module_name === 'admissions') $allowed = true;
    
    if (!$allowed) { die("غير مصرح"); }
}
...
?>
            ...
            <li><a href="change_password.php" class="active">تغيير كلمة السر</a></li>
        </ul>
